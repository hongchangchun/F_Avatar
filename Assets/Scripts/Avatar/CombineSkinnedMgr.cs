...
// 	private const string COMBINE_DIFFUSE_TEXTURE = "_MainTex";
	...
	public static void CombineObject (GameObject skeleton, SkinnedMeshRenderer[] meshes, bool combine = false, int combine_text_size = 512){
		...
		if (combine)
		{
            ...
            newMaterial = new Material(materials[0]);
            oldUV = new List<Vector2[]>();

            List<string> texture_names_list = new List<string>();
            newMaterial.GetTexturePropertyNames(texture_names_list);
            Rect[] uvs = null;

            //贴图的合并：如果只有一份主贴图，就是最容易支持的，其它各种贴图布局得跟主贴图一样了，不然UV只有一份，就无法应付了
            for (int t = 0; t < texture_names_list.Count; t++)
            {
                string tex_name = texture_names_list[t];
                // merge the texture
                List<Texture2D> Textures = new List<Texture2D>();
                for (int i = 0; i < materials.Count; i++)
                {
                    //Textures.Add(materials[i].GetTexture(COMBINE_DIFFUSE_TEXTURE) as Texture2D);
                    Texture2D tex = materials[i].GetTexture(tex_name) as Texture2D;
                    if (tex == null)
                        break;
                    Textures.Add(tex);//*02
                }
                if (Textures.Count > 0)
                {
                    newDiffuseTex = new Texture2D(combine_text_size, combine_text_size, TextureFormat.RGBA32, true);
                    uvs = newDiffuseTex.PackTextures(Textures.ToArray(), 0);
                    //newMaterial.mainTexture = newDiffuseTex;
                    newMaterial.SetTexture(tex_name, newDiffuseTex);
                }
            }
